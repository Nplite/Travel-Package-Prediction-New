import os, sys
import Optional
from Travel_Package_prediction.entity.config_entity import TRANSFORMER_OBJECT_FILE_NAME,MODEL_FILE_NAME, TARGET_ENCODER_OBJECT_FILE_NAME
# Folder (new model saved)
# Comparision of new model with old model
# the model accepted or rejected
# save model  - > 0 ->1 ->2 ->3 ->4 in this way t will be created folders
...
class ModelResolver:
    ...
    def get_latest_transformer_path(self):
        try:
            latest_dir = self.get_latest_dir_path()
            if latest_dir is None:
                raise Exception(f"Transformed data is not available")
    ...
    def get_latest_save_model_path(self):
        try:
            ...
            return os.path.join(latest_dir, self.model_dir_name, MODEL_FILE_NAME)       # model.pkl file
        except Exception as e:
            raise e
